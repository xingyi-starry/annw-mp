...
协议现为 v13，房间消息增加主机提供的压缩地图预览。`WireEnvelope` 只携带消息类型、payload 与路由身份，协议版本只存在于 `Hello.protocol_version`，由游戏主机校验。Relay 仅解析自身控制消息，其他 payload 透明转发，无需因地图预览字段改变路由逻辑。v13 插件不能与旧协议插件混用。
